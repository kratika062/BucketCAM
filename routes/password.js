...
// forgot password
router.get("/password_reset", (req, res) => res.render("bucket/password_reset"));
  ...
  asyn.waterfall([
      ...
      User.findOne({ email: req.body.email }, (err, user) => {
        ...
        if (!user) {
          req.flash("error", "That YelpCamp account doesn't exist.");
			return res.send("THAT BucketCAM ACCOUNT DOESN'T EXISTS");
    ...
    function(token, user, done) {
      ...
      let smtpTransport = nodemailer.createTransport({
        service: "Gmail",
        auth: {
          user:"kratikajn6@gmail.com",
          pass:process.env.PASSWORD
        }
      });
      let mailOptions = {
        from: "kratikajn6@gmail.com",
        to: user.email,
...
router.get("/reset/:token", (req, res) => {
  User.findOne({ resetPasswordToken: req.params.token, resetPasswordExpires: { $gt: Date.now() } }, (err, user) => {
    ...
    if (!user) {
      ...
      res.redirect("/password_reset");
    } else { res.render("bucket/reset", { token: req.params.token }) }
  });
});
